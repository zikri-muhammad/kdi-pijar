...
@section('content')
    ...
    <div class="row">
      <div class="col-md-6 mx-auto pt-5 mt-5">
				...
				<h5 class="mb-4 text-center">Silahkan Lengkapi Formulir Berikut</h5>
				<div class="card card-body">
					{{-- <form class="mt-4" action="/signin" novalidate> --}}
					<form class="mt-4" action="{{ route('activation.form') }}" novalidate method="POST">
						@csrf
						{{-- <div class="form-group">
							<label>NIS / NIP:</label>
							<input type="text" name="nis" class="form-control" value="{{ Session::get('users')['nis'] }}" readonly>
						</div>
            			<div class="form-group">
							<label>Nama:</label>
							<input type="text" name="name" class="form-control" value="{{ Session::get('users')['name'] }}" readonly>
						</div>
            			<div class="form-group">
							<label>Kelas:</label>
							<input type="text" name="class" class="form-control" value="{{ Session::get('users')['class_name'] }}" readonly>
						</div> --}}
            			<div class="form-group">
							<label>Email:</label>
							<input type="email" name="email" class="form-control" value="{{ Session::get('users')['email'] }}" placeholder="Masukkan Email" readonly>
						</div>
            			<div class="form-group">
            			...
            			<div class="form-group text-center">
							<button type="submit" class="btn btn-primary px-5">Aktivasi Akun</button>
						</div>
						<div class="form-group text-center mb-0">
							<small>
								Sudah aktivasi akun? <a class="text-underline" href="/login">Login sekarang!</a>
							</small>
						</div>
					</form>		
				</div>
